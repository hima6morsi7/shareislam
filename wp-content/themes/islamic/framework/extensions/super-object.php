<?php
	...
	*/
			...
			foreach($xml->childNodes as $xmlChild){
				...
				if($xmlChild->nodeName == $field){
					if( is_admin() ){
						...
						return __($xmlChild->nodeValue, 'cp_front_end');
					}
				}
	...
	add_action('wp_ajax_submit_contact_form','cp_submit_contact_form');
	add_action('wp_ajax_nopriv_submit_contact_form','cp_submit_contact_form');
	function cp_submit_contact_form(){
		...
		if( @mail($receiver, 'New contact form received', $messages, $header) ){
			...
			$return_data['value'] = __('The e-mail was sent successfully','cp_front_end');
			die( json_encode($return_data) );
		}else{
			$return_data['value'] = __('Message cannot be sent to destination','cp_front_end');
			die( json_encode($return_data) );	
		}
		
	}
	
	add_action('wp_ajax_load_dummy_data','cp_load_dummy_data');
	function cp_load_dummy_data(){
		
		require_once ABSPATH . 'wp-admin/includes/import.php';
		
		$import_path = get_template_directory() . "/framework/plugin/dummy-data.xml";
		$import_error = false;
