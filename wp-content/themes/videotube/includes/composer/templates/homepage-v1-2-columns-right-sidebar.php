...
if( !function_exists( 'mars_home_v1_2cols_right_sidebar' ) ){
	function mars_home_v1_2cols_right_sidebar($data) {
		$template               = array();
		$template['name']       = __( 'Homepage V1 - 2 Cols- Right Sidebar', 'mars' );
		...
		$template['custom_class'] = 'mars_home_v1_2cols_right_sidebar';
		$template['content']    = <<<CONTENT
		[vc_row][vc_column width="1/1"][vc_row_inner][vc_column_inner width="8/12"][videotube type="main" show="18" id="video-widget-3911" rows="1" columns="2" navigation="on" title="Recent Videos" orderby="ID" order="DESC"][/vc_column_inner][vc_column_inner width="4/12" el_class="sidebar"][mars_vc_socialsbox title="Socials Box"][videotube type="widget" show="3" id="video-widget-3338" rows="1" columns="1" title="Latest Videos" orderby="ID" order="DESC"][vc_wp_text title="ABOUT VIDEOTUBE"]Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics, a large language ocean which is great.
...
CONTENT;
	...
	add_filter( 'vc_load_default_templates', 'mars_home_v1_2cols_right_sidebar', 30, 1 );
}
