<?php
	...
	*/
	...
	function print_item_size($item_size, $addition_class=''){
		...
		switch($item_size){
			case 'element1-4':
				echo '<article class="four columns ' . $addition_class . '">';
				$cp_item_row_size += 1/4; 
				break;
			case 'element1-3':
				echo '<article class="one-third column ' . $addition_class . '">';
				$cp_item_row_size += 1/3; 
				break;
			case 'element1-2':
				echo '<article class="eight columns ' . $addition_class . '">';
				$cp_item_row_size += 1/2; 
				break;
			case 'element2-3':
				echo '<article class="two-thirds column ' . $addition_class . '">';
				$cp_item_row_size += 2/3; 
				break;
			case 'element3-4':
				echo '<article class="twelve columns ' . $addition_class . '">';
				$cp_item_row_size += 3/4; 
				break;
			case 'element1-1':
				echo '<article class="sixteen columns ' . $addition_class . '">';
				$cp_item_row_size += 1; 
				break;	
		}
	...
	function print_gallery_item($item_xml){
		...
		if(!empty($header)){
			echo '<h3 class="gallery-header-title title-color cp-title">' . $header . '<span class="h-line"></span></h3>';
		}	
	...
	function print_slider_item($item_xml){
		...
		switch(find_xml_value($item_xml,'slider-type')){
			...
			case 'Refine Slider': 
				print_refine_slider(find_xml_node($item_xml,'slider-item'), $xml_size); 
				break;
	...
	function print_divider($item_xml){
		...
		echo '<div class="divider mr10"><div class="scroll-top">';
	...
	function print_tab_item($item_xml){
		...
		echo '<span class="h-line"> </span></h3>';
		}
		foreach($tab_xml->childNodes as $toggle_box){
	...
	function print_price_item($item_xml){
		...
		foreach($price_posts as $price_post){
			...
			echo '<div class="price-tag">' . __(get_post_meta( $price_post->ID, 'price-table-price-tag', true ), 'cp_front_end') . '</div>';
			...
			$price_url = __(get_post_meta( $price_post->ID, 'price-table-option-url', true ), 'cp_front_end');
			if( !empty($price_url) ){
	...
	function print_column_service($item_xml){
		...
		if(!empty($column_service_image)){
		    echo '<a href="'.$column_service_link.'">';
			echo "<img src='" . $column_service_image[0] . "' alt='" . $alt_text ."' class='column-service-image'/>";
			echo '</a>';
		}
		echo '<p>'.do_shortcode($column_service_caption).'';
		echo '</p>';
		if(!empty($column_service_link)){echo'<a class="txt-btn" href="'.$column_service_link.'">'.__('More+','crunchpress').'</a>';}
		echo '</div>';
		}else{
		...
		echo '<h3 class="column-service-title cp-title">' . $column_service_title . '<span class="h-line">&nbsp;</span></h3>';
		echo '<p>'.do_shortcode($column_service_caption).'';
		echo '</p>';
		if(!empty($column_service_link)){echo'<a class="cp-button" href="'.$column_service_link.'">'.__('View More...','crunchpress').'</a>';}
		echo '</div>';
...
$cp_div_size_num_class = array("1/4" => "four columns", "1/3" => "one-third column", "1/2" => "eight columns", 	"2/3" => "two-thirds column", "3/4" => "twelve columns", "1/1" => "sixteen columns");
	...
	function print_testimonial($item_xml){
		...
		if($display_type == 'Specific Testimonial'){
			...
			if(!empty($header)){
				echo '<h3 class="testimonial-header-title title-color cp-title">' . $header . '<span class="h-line">&nbsp;</span></h3>';
			}
			...
			echo '</div>';
			$position = __(get_post_meta($posts[0]->ID, 'testimonial-option-author-position', true), 'cp_front_end');
			...
			foreach( $category_posts as $category_post){
				echo '<li class="' . $cp_div_size_num_class[$item_size] .' mt0">';
				echo '<div class="testimonial-content">';
				echo '<div class="testimonial-icon"></div>';
				echo $category_post->post_content;
				echo '</div>';
				$position = __(get_post_meta($category_post->ID, 'testimonial-option-author-position', true), 'cp_front_end');
				echo '<div class="testimonial-author cp-divider">';
	...
	function print_portfolio($item_xml){
		...
		/*if(!empty($header)){
			echo '<div class="portfolio-header-wrapper columns"><h3 class="portfolio-header-title title-color cp-title">' . $header . '</h3></div>';
                        ...
                        echo '<div class="portfolio-header-wrapper"><h3 class="portfolio-header-title title-color cp-title">Portfolio Items<span class="h-line"></span></h3></div>';
                }*/  
		...
		while( have_posts() ){
			...
			if( $thumbnail_types == "Image" ){
				...
				if( $image_type == "Link to Current Post" ){
					...
					$hover_thumb = "hover-link";
					$pretty_photo = "";
					$permalink = __(get_post_meta( $post->ID, 'post-option-featured-image-url', true ), 'cp_front_end');
				...
				if( !empty($thumbnail[0]) ){
					...
					/*echo '<div class="port-shadow"><img src="'.CP_PATH_URL.'/images/port-shadow.png" alt="'. $alt_text .'"/></div>';*/
		...
		if( $sidebar == "no-sidebar" ){
			...
			if( find_xml_value($item_xml, "read-more") == "Yes" ){
				echo '<a href="' . get_permalink() . '" class="more-btn">' . __('+ More','cp_front_end') . '</a>';
			}
	...
	function print_product_slider_item($item_xml){
		...
		$category = find_xml_value($item_xml, 'category');
		$category = ($category == 'All')? '': $category;
        echo do_shortcode('[wcomm_carrousel orderby="date" order="'.$product_order.'" size="200" filter="all" category="'.$category.'"]');		
		}else {
