...
<?php
		...
		if( $cp_admin_translator == 'enable' ){
			$translator_client = get_option(THEME_NAME_S.'_translator_client', 'Client');
			$translator_visit_website = get_option(THEME_NAME_S.'_translator_visit_website', 'Visit Website');
			$translator_about_author = get_option(THEME_NAME_S.'_translator_about_author', 'About the Author');
			$translator_social_share = get_option(THEME_NAME_S.'_translator_social_shares', 'Social Share');
		}else{
			$translator_client =  __('Client','cp_front_end');
			$translator_visit_website = __('Visit Website','cp_front_end');		
			$translator_about_author = __('About the Author','cp_front_end');
			$translator_social_share = __('Social Share','cp_front_end');
		}		
						...
						<div class='cp-page-item'>
							<?php 
									if ( have_posts() ){ while (have_posts()){ the_post();
											...
											if( get_option(THEME_NAME_S.'_use_portfolio_as') == 'portfolio style' ){
												...
												echo '<div class="single-port-date"><span class="head">' . __('Date:','cp_front_end') . '</span>';
												echo get_the_time( GDL_DATE_FORMAT ) . '</div>';
												
												$portfolio_tag = get_the_term_list( $post->ID, 'portfolio-tag', '', ', ' , '' );
												if(!empty($portfolio_tag)){
												echo '<div class="single-port-tag"><span class="head">' . __('Tags:','cp_front_end') . '</span>';
												echo $portfolio_tag;
												echo '</div>';
												}
												...
												echo '<div class="twelve columns mt0">';
													echo '<h1 class="single-port-title post-title-color cp-title cp-divider">';
														echo '<a href="' . get_permalink() . '">' . get_the_title() . '</a><span class="h-line"></span></h1>';
												...
												// Single header
												echo '<h1 class="single-thumbnail-title post-title-color cp-title"><a href="' . get_permalink() . '">' . get_the_title() . '</a></h1>';
												echo '<div class="single-thumbnail-info post-info-color cp-divider">';
													...
													echo '<div class="single-thumbnail-author"> ' . __('by','cp_front_end') . ' ' . get_the_author_link() . '</div>';
														$portfolio_tag = get_the_term_list( $post->ID, 'portfolio-tag', '', ', ' , '' );
													echo '<div class="single-thumbnail-tag">' . $portfolio_tag . '</div>';
													
													echo '<div class="single-thumbnail-comment">';
														comments_popup_link( __('0 Comment','cp_front_end'), __('1 Comment','cp_front_end'),
															__('% Comments','cp_front_end'), '', __('Comments are off','cp_front_end') );
													echo '</div>';
													echo '<div class="clear"></div>';
												echo '</div>';
