<?php
/**
 * Plugin Name: CrunchPress Recent Post
...
class Recent_Post extends WP_Widget {
	...
	function Recent_Post() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'cp_recentpost-widget', __(THEME_NAME_F. ' - Recent Posts', 'crunchpress'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( !empty($custom_posts) ){ 
			echo "<div class='cp-recent-post-widget'>";
				...
				<div class="recent-post-widget">
					<div class="recent-post-widget-thumbnail">
						<a href="<?php echo get_permalink( $custom_post->ID ); ?>">
							<?php
								...
								$thumbnail = wp_get_attachment_image_src( $thumbnail_id , '70x45' );	
					...
					<div class="recent-post-widget-context">
						<div class="recent-post-widget-title">
							<a href="<?php echo get_permalink( $custom_post->ID ); ?>"> 
								<?php _e( $custom_post->post_title, 'cp_front_end'); ?> 
							</a>
						</div>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Recent Posts Widget', 'crunchpress'), 'post_cat' => __('All', 'crunchpress'), 'show_num' => '3');
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_cat' ); ?>"><?php _e('Category :', 'crunchpress'); ?></label>		
