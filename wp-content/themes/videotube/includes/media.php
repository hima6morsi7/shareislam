...
if( !function_exists( 'mediapress_get_embedcode' ) ){
	...
	function mediapress_get_embedcode( $media_object, $post_id ) {
		...
		if( mediapress_get_video_type($post_id) == 'normal'){
			...
			global $mars_twitter, $mars_openload, $mars_facebook, $mars_streamable;
			...
			if( $embed_html = $mars_twitter->get_iframe( array( 'src' => $media_object ))){
				$output = $embed_html;
			}
			elseif( $embed_html = $mars_openload->get_iframe( array( 'src' => $media_object ))){
				$output = $embed_html;
			}
			elseif( $embed_html = $mars_facebook->get_iframe( array( 'src' => $media_object ))){
				$output = $embed_html;
			}
			elseif( $embed_html = $mars_streamable->get_iframe( array( 'src' => $media_object ))){
				$output = $embed_html;
			}
			elseif( $embed_html = wp_oembed_get( $media_object, $mediapress_object_url_args ) ){
				$output = $embed_html;
			}
			else{
				$output .= $media_object;
