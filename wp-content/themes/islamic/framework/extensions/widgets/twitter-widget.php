<?php
/**
 * Plugin Name: Crunchpress Twitter Widget
...
class Cp_Twitter_Widget extends WP_Widget {

	function Cp_Twitter_Widget() {
		$widget_ops = array('classname' => 'widget_twitter', 'description' => __( 'Display your tweets from Twitter','crunchpress') );
		parent::WP_Widget('twitter', __(THEME_NAME_F. ' - Twitter', 'crunchpress'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty($title) ) $title = __( 'Twitter Updates','crunchpress');
	...
	function form( $instance ) {
		//Defaults
		$instance = wp_parse_args( (array) $instance, array('account' => '', 'title' => '', 'show' => 3, 'hidereplies' => false) );
...
add_action( 'widgets_init', 'cp_twitter_widget_init' );
function cp_twitter_widget_init() {
	register_widget('Cp_Twitter_Widget');
}